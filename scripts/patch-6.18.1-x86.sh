...
#############################################
# Kernel konfigurációs fragment előkészítés
# Kernel config fragment preparation
#############################################
...
: "${CHROOT_DIR:?CHROOT_DIR nincs beállítva}"
: "${DISTRO_COMPAT_VERSION:?DISTRO_COMPAT_VERSION nincs beállítva}"
: "${KERNEL_VERSION:?KERNEL_VERSION nincs beállítva}"
: "${KERNEL_FLAVOUR:?KERNEL_FLAVOUR nincs beállítva}"

#############################################
# Kernel config könyvtár (meglévő struktúra!)
# Kernel config directory (existing structure)
#############################################

KCFG_DIR="$CHROOT_DIR/DISTRO_CONFIG/${DISTRO_COMPAT_VERSION}_${KERNEL_VERSION}_${KERNEL_FLAVOUR}"
KERNEL_CFG="$KCFG_DIR/kernel_config.cfg"

mkdir -p "$KCFG_DIR"

echo "=============================================="
echo " Kernel konfigurációs fragment létrehozása"
echo " Kernel config fragment creation"
echo "----------------------------------------------"
echo "Kernel verzió : $KERNEL_VERSION"
echo "Kernel flavour: $KERNEL_FLAVOUR"
echo "Cél könyvtár  : $KCFG_DIR"
echo "Fájl          : kernel_config.cfg"
echo "=============================================="
echo ""
...
• Ez egy kernel konfigurációs fragment (.cfg)
• KuLuSz merge logikájához készült
• Az első 4 sor fejléc (header)
• A merge során EZ A 4 SOR NEM KERÜL BE
• Több .cfg fájl is elhelyezhető ebben a könyvtárban
• A kernel letöltése UTÁN még módosítható
• A kernel fordítás UTÁN már NEM

INFO

#############################################
# kernel_config.cfg létrehozása
#############################################

echo "kernel_config.cfg létrehozása:"
echo "  $KERNEL_CFG"
...
#
CONFIG_CC_VERSION_TEXT="gcc-14.1.0 (GCC) 14.1.0"
CONFIG_CC_IS_GCC=y
...
echo ""
echo "Kernel konfigurációs fragment elkészült:"
echo "  $KERNEL_CFG"
echo ""
